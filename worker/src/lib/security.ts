...
export function validatePasswordStrength(password: string): string | null {
  ...
  if (password.length > 128) {
    return 'Senha muito longa (máx. 128 caracteres)';
  }
...
export async function recordLoginAttempt(
  ...
  try {
    await env.DB.prepare(
      ...
      .bind(email.toLowerCase(), success ? 1 : 0, ip || null)
      .run();
    ...
    if ((result?.count ?? 0) >= 5) {
      ...
      await env.DB.prepare(
        ...
        .bind(email.toLowerCase(), lockedUntil)
        .run();
    }
  } catch {
...
export async function auditLog(
  ...
  try {
    await env.DB.prepare(
      ...
      .bind(userId, action, JSON.stringify(details), ip || null)
      .run();
  } catch {
