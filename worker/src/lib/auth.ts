...
export async function requireAuth(
  ...
  if (!verified.valid || !verified.payload) {
    return {
      ok: false,
      response: json({ success: false, error: 'Token inválido ou expirado' }, 401),
    };
  }
  ...
  if (!session || session.user_id !== verified.payload.sub) {
    return {
      ok: false,
      response: json({ success: false, error: 'Sessão revogada ou inexistente' }, 401),
    };
  }
...
export async function enable2FA(
  ...
  await env.DB.prepare(
    ...
    .bind(secret, JSON.stringify(backupCodes), userId)
    .run();
}
