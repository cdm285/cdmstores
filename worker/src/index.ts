...
import { handleGuardianReport, handleGuardianStatus } from './routes/guardian.js';
import { handleOrderCreate, handleOrderGet } from './routes/orders.js';
import { handleOrganicRequest } from './routes/organic.js';
import { handleProductGet, handleProductList } from './routes/products.js';
...
async function handleRequest(request: Request, env: Env): Promise<Response> {
    ...
    return json(
      {
        ...
        bindings: {
          ...
          kv_rate_limit: !!env.RATE_LIMIT,
          kv_metrics: !!env.METRICS,
        },
        metrics: metrics ?? {},
        circuits,
      },
  ...
  if (path === '/api/auth/forgot-password' && m === 'POST') {
    return handleForgotPassword(request, env);
  }
  if (path === '/api/auth/reset-password' && m === 'POST') {
    return handleResetPassword(request, env);
  }
  if (path === '/api/auth/send-verification-email' && m === 'POST') {
    return handleSendVerificationEmail(request, env);
  }
  if (path === '/api/auth/verify-email' && m === 'POST') {
    return handleVerifyEmail(request, env);
  }
  if (path === '/api/auth/change-password' && m === 'POST') {
    return handleChangePassword(request, env);
  }
  if (path === '/api/auth/google' && m === 'POST') {
    return handleGoogleAuth(request, env);
  }
  if (path === '/api/auth/facebook' && m === 'POST') {
    return handleFacebookAuth(request, env);
  }

  // ── 2FA ──────────────────────────────────────────────────────────────────
  if (path === '/api/auth/2fa/setup' && m === 'POST') {
    return handle2FASetup(request, env);
  }
  if (path === '/api/auth/2fa/verify-setup' && m === 'POST') {
    return handle2FAVerifySetup(request, env);
  }
  if (path === '/api/auth/2fa/disable' && m === 'POST') {
    return handle2FADisable(request, env);
  }
  if (path === '/api/auth/2fa/verify' && m === 'POST') {
    return handle2FAVerify(request, env);
  }

  // ── USER / ADDRESSES ─────────────────────────────────────────────────────
  if (path === '/api/user/profile' && m === 'PUT') {
    return handleUpdateProfile(request, env);
  }
  if (path === '/api/addresses' && m === 'GET') {
    return handleGetAddresses(request, env);
  }
  if (path === '/api/addresses' && m === 'POST') {
    return handleCreateAddress(request, env);
  }
  if (path === '/api/orders/user' && m === 'GET') {
    return handleUserOrders(request, env);
  }
  ...
  if (addressMatch) {
    ...
    if (m === 'PUT' && !isDefault) {
      return handleUpdateAddress(request, env, id);
    }
    if (m === 'DELETE' && !isDefault) {
      return handleDeleteAddress(request, env, id);
    }
    if (m === 'POST' && isDefault) {
      return handleSetDefaultAddress(request, env, id);
    }
  }

  // ── PRODUCTS ─────────────────────────────────────────────────────────────
  if (path === '/api/products' && m === 'GET') {
    return handleProductList(request, env);
  }
  const productMatch = path.match(/^\/api\/products\/(\d+)$/);
  if (productMatch && m === 'GET') {
    return handleProductGet(request, env, productMatch[1]);
  }
  ...
  if (orderMatch && m === 'GET') {
    return handleOrderGet(request, env, orderMatch[1]);
  }

  // ── CART ─────────────────────────────────────────────────────────────────
  if (path === '/api/cart/add' && m === 'POST') {
    return handleCartAdd(request, env);
  }

  // ── TRACKING ─────────────────────────────────────────────────────────────
  const trackingMatch = path.match(/^\/api\/tracking\/([^/]+)$/);
  if (trackingMatch && m === 'GET') {
    return handleTracking(request, env, trackingMatch[1]);
  }

  // ── STRIPE ───────────────────────────────────────────────────────────────
  if (path === '/api/stripe/create-payment' && m === 'POST') {
    return handleStripeCreatePayment(request, env);
  }
  ...
  // ── GUARDIAN ─────────────────────────────────────────────────────────────
  if (path === '/api/guardian/status' && m === 'GET') {
    return handleGuardianStatus(request, env);
  }
  if (path === '/api/guardian/report' && m === 'GET') {
    return handleGuardianReport(request, env);
  }
  ...
  if (path.startsWith('/api/ai/')) {
    return handleWorkersAI(request, env, path);
  }
  ...
  scheduled: async (_controller: ScheduledController, env: Env) => {
    ...
    try {
      await env.DB.batch([
        ...
        env.DB.prepare(
          "DELETE FROM two_factor_attempts WHERE created_at < datetime('now', '-1 day')",
        ),
      ]);
      dbOk = true;
      logger.log('[GUARDIÃO:cron] DB cleanup OK');
    } catch (err) {
      logger.error('[GUARDIÃO:cron] DB cleanup FAILED:', err instanceof Error ? err.message : err);
    }
    ...
    try {
      ...
      if (env.METRICS) {
        ...
        bucket.push({
          ...
          path: '/__cron__',
          method: 'CRON',
          status: probe ? 200 : 503,
          latencyMs,
          requestId: crypto.randomUUID(),
        });
        await env.METRICS.put(key, JSON.stringify(bucket.slice(-500)), { expirationTtl: 93_600 });
      }
      logger.log(`[GUARDIÃO:cron] self-probe OK — db:${dbOk} latency:${Date.now() - ts}ms`);
    } catch (err) {
      logger.error('[GUARDIÃO:cron] self-probe FAILED:', err instanceof Error ? err.message : err);
    }
  },
