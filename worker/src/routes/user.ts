...
export async function handleSetDefaultAddress(req: Request, env: Env, addressId: string): Promise<Response> {
  try {
    ...
    await env.DB.prepare('UPDATE user_addresses SET is_default = 0 WHERE user_id = ?').bind(authResult.auth.userId).run();
    await env.DB.prepare('UPDATE user_addresses SET is_default = 1 WHERE id = ?').bind(addressId).run();
