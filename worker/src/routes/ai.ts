...
router.post('/chat', async (req, env: Env) => {
  try {
    ...
    const history = (await env.DB.prepare(
      ...
      .bind(conversationId)
      .all()) as { results: Array<{ role: string; content: string }> };

    const messages = [
      {
        role: 'system' as const,
        content:
          'Você é o assistente da CDM STORES, uma loja online especializada em produtos premium. Seja prestativo, conciso e foque em ajudar o cliente com compras, rastreamento de pedidos e suporte.',
      },
      ...history.results
        .reverse()
    ...
    let semanticContext = '';
    try {
      const queryEmbedding = (await env.AI.run(MODELS.embedding, { text: [message] })) as {
        data: number[][];
      };
    ...
    await env.DB.batch([
      ...
      env.DB.prepare('UPDATE ai_conversations SET updated_at = datetime("now") WHERE id = ?').bind(
        conversationId,
      ),
    ]);
    ...
    try {
      ...
      const embedding = (await env.AI.run(MODELS.embedding, { text: [contentToEmbed] })) as {
        data: number[][];
      };
      if (embedding.data?.[0]) {
...
router.post('/search', async (req, env: Env) => {
  try {
    ...
    const embedding = (await env.AI.run(MODELS.embedding, { text: [query] })) as {
      data: number[][];
    };
...
router.post('/classify', async (req, env: Env) => {
  try {
    ...
    const result = (await env.AI.run(MODELS.classification, { text })) as unknown as Array<{
      label: string;
      score: number;
    }>;
...
router.post('/image', async (req, env: Env) => {
  try {
    ...
    const result = (await env.AI.run(MODELS.image_gen, {
      prompt,
...
router.post('/transcribe', async (req, env: Env) => {
  try {
    ...
    if (!audioFile) {
      return json(
        { success: false, error: 'arquivo de áudio obrigatório (campo "audio")' },
        { status: 400 },
      );
    }
    ...
    categories: {
      text_generation: {
        recommended: [
          ...
          {
            key: 'llama4_scout',
            id: MODELS.llama4_scout,
            desc: 'Llama 4 Scout 17B - Vision + Function calling',
          },
        ...
        reasoning: [
          ...
          { key: 'qwq_32b', id: MODELS.qwq_32b, desc: 'QwQ 32B - raciocínio' },
          {
            key: 'kimi_k2_5',
            id: MODELS.kimi_k2_5,
            desc: 'Kimi K2.5 - 256K ctx, Vision (Partner)',
          },
        ...
        specialized: [
          ...
          {
            key: 'granite_micro',
            id: MODELS.granite_micro,
            desc: 'IBM Granite 4.0 Micro - Function calling',
          },
          ...
          {
            key: 'mistral_small',
            id: MODELS.mistral_small,
            desc: 'Mistral Small 3.1 24B - Vision, 128K ctx',
          },
          ...
          {
            key: 'llama_guard',
            id: MODELS.llama_guard,
            desc: 'Llama Guard 3 8B - segurança de conteúdo',
          },
        ],
      },
      text_to_image: [
        ...
        {
          key: 'image_flux2_4b',
          id: MODELS.image_flux2_4b,
          desc: 'FLUX.2 Klein 4B - ultra-rápido (Partner)',
        },
        {
          key: 'image_flux2_9b',
          id: MODELS.image_flux2_9b,
          desc: 'FLUX.2 Klein 9B - alta qualidade (Partner)',
        },
        {
          key: 'image_flux2_dev',
          id: MODELS.image_flux2_dev,
          desc: 'FLUX.2 Dev - multi-referência (Partner)',
        },
        { key: 'image_sdxl', id: MODELS.image_sdxl, desc: 'Stable Diffusion XL Base 1.0' },
        {
          key: 'image_sdxl_lightning',
          id: MODELS.image_sdxl_lightning,
          desc: 'SDXL Lightning - rápido',
        },
        {
          key: 'image_dreamshaper',
          id: MODELS.image_dreamshaper,
          desc: 'Dreamshaper 8 LCM - fotorrealismo',
        },
      ...
      text_to_speech: [
        { key: 'tts_en', id: MODELS.tts_en, desc: 'Deepgram Aura-2 EN - inglês (Partner)' },
        {
          key: 'tts_es',
          id: MODELS.tts_es,
          desc: 'Deepgram Aura-2 ES - espanhol/português (Partner)',
        },
      ...
      speech_recognition: [
        ...
        {
          key: 'speech_to_text_turbo',
          id: MODELS.speech_to_text_turbo,
          desc: 'Whisper Large v3 Turbo - rápido',
        },
        {
          key: 'speech_nova3',
          id: MODELS.speech_nova3,
          desc: 'Deepgram Nova-3 (Partner, Real-time)',
        },
        {
          key: 'speech_deepgram_flux',
          id: MODELS.speech_deepgram_flux,
          desc: 'Deepgram Flux - agentes de voz (Partner)',
        },
      ],
      embeddings: [
        ...
        {
          key: 'embedding_gemma',
          id: MODELS.embedding_gemma,
          desc: 'EmbeddingGemma 300M - 100+ idiomas',
        },
        { key: 'embedding_qwen3', id: MODELS.embedding_qwen3, desc: 'Qwen3 Embedding 0.6B' },
        {
          key: 'embedding_plamo',
          id: MODELS.embedding_plamo,
          desc: 'PLaMo Embedding 1B - japonês',
        },
      ...
      other: [
        {
          key: 'classification',
          id: MODELS.classification,
          desc: 'DistilBERT SST-2 - classificação de sentimento',
        },
        {
          key: 'image_to_text',
          id: MODELS.image_to_text,
          desc: 'LLaVA 1.5 7B - imagem para texto',
        },
        {
          key: 'object_detection',
          id: MODELS.object_detection,
          desc: 'DETR ResNet-50 - detecção de objetos',
        },
        { key: 'image_class', id: MODELS.image_class, desc: 'ResNet-50 - classificação de imagem' },
        {
          key: 'summarization',
          id: MODELS.summarization,
          desc: 'BART Large CNN - resumo de texto',
        },
        { key: 'translation', id: MODELS.translation, desc: 'M2M100 1.2B - tradução 100+ idiomas' },
        {
          key: 'translation_indic',
          id: MODELS.translation_indic,
          desc: 'IndicTrans2 - EN para 22 idiomas indicos',
        },
