...
import { buildSetCookieHeaders, internalError, json, jsonWithCookies } from '../lib/response.js';
import { auditLog, checkRateLimit } from '../lib/security.js';
...
export async function handle2FASetup(req: Request, env: Env): Promise<Response> {
  try {
    ...
    return json({
      ...
      qrCodeUrl: qrUrl,
      message:
        'Autenticador configurado. Escaneie o código QR com seu app de autenticação (Google Authenticator, Authy, etc.)',
    });
  } catch (error) {
...
export async function handle2FAVerifySetup(req: Request, env: Env): Promise<Response> {
  try {
    ...
    const { code, secret, backupCodes } = body as {
      code?: string;
      secret?: string;
      backupCodes?: string[];
    };
    ...
    const safeBackupCodes =
      Array.isArray(backupCodes) && backupCodes.length > 0 ? backupCodes : generateBackupCodes(10);
...
export async function handle2FAVerify(req: Request, env: Env): Promise<Response> {
  try {
    ...
    const body = (await req.json()) as Record<string, unknown>;
    const { challengeToken, code, backupCode } = body as {
    ...
    if (!challengeToken) {
      return json({ success: false, error: 'challengeToken obrigatório' }, 400);
    }
    if (!code && !backupCode) {
    ...
    if (code) {
      ...
      if (recentlyUsed) {
        return json(
          { success: false, error: 'Código já utilizado. Aguarde o próximo código.' },
          401,
        );
      }
    ...
    if (!isValid && backupCode) {
      try {
        ...
        logger.error('Error parsing backup codes:', e instanceof Error ? e.message : e);
      }
    }
