...
async function calculateOrderTotal(
  ...
  total: number;
  enrichedItems: Array<{ product_id: number; quantity: number; price: number; name: string }>;
} | null> {
  let subtotal = 0;
...
export async function handleOrderGet(req: Request, env: Env, id: string): Promise<Response> {
  try {
    ...
    const order = await env.DB.prepare(
      ...
      .bind(Number(id), authResult.auth.userId)
      .first();
    if (!order) {
...
export async function handleOrderCreate(req: Request, env: Env): Promise<Response> {
  try {
    ...
    await auditLog(env, authResult.auth.userId, 'order_created', {
      order_id: orderId,
      total: calculated.total,
    });
    return json(
      { success: true, order_id: orderId, total: calculated.total, status: 'pending' },
      201,
    );
  } catch (error) {
