...
export async function handleRegister(req: Request, env: Env): Promise<Response> {
  try {
    ...
    if (passError) {
      return json({ success: false, error: passError }, 400);
    }
    ...
    await env.DB.prepare(
      ...
      .bind(userId, hashToken(verificationToken), new Date(Date.now() + 86400 * 1000).toISOString())
      .run();
...
export async function handleRefresh(req: Request, env: Env): Promise<Response> {
  try {
    ...
    const session = await env.DB.prepare(
      ...
      .bind(hashToken(refreshToken))
      .first<{ user_id: number; refresh_expires_at: string }>();
    ...
    if (
      ...
      session.refresh_expires_at <= new Date().toISOString()
    ) {
    ...
    return jsonWithCookies(
      ...
      200,
      buildSetCookieHeaders(rotated.token, rotated.refreshToken),
    );
  } catch (error) {
...
export async function handleForgotPassword(req: Request, env: Env): Promise<Response> {
  try {
    ...
    await env.DB.prepare(
      ...
      .bind(user.id, hashToken(resetToken), expiresAt)
      .run();
...
export async function handleResetPassword(req: Request, env: Env): Promise<Response> {
  try {
    ...
    const resetRecord = await env.DB.prepare(
      ...
      .first<{ user_id: number; expires_at: string; used: number }>();
...
export async function handleSendVerificationEmail(req: Request, env: Env): Promise<Response> {
  try {
    ...
    return json({
      success: true,
      message: emailSent ? 'Email de verificação enviado' : 'Usuário marcado para verificação',
    });
  } catch (error) {
...
export async function handleVerifyEmail(req: Request, env: Env): Promise<Response> {
  try {
    ...
    const resetRecord = await env.DB.prepare(
      ...
      .bind(hashToken(token), verified.userId)
...
export async function handleChangePassword(req: Request, env: Env): Promise<Response> {
  try {
    ...
    if (!passwordMatch) {
      await auditLog(env, authResult.auth.userId, 'password_change_failed', {
    ...
    await env.DB.prepare(
      ...
      .bind(newHash, authResult.auth.userId)
      .run();

    await env.DB.prepare('DELETE FROM sessions WHERE user_id = ? AND token != ?')
      .bind(authResult.auth.userId, hashToken(authResult.auth.token))
      .run();
  ...
  try {
    ...
    try {
      if (idToken) {
        ...
        const info = (await infoResp.json()) as Record<string, string>;
        if (!infoResp.ok) {
          return json({ success: false, error: 'Access token Google inválido' }, 401);
        }
        if (
          env.GOOGLE_CLIENT_ID &&
          info.issued_to !== env.GOOGLE_CLIENT_ID &&
          info.audience !== env.GOOGLE_CLIENT_ID
        ) {
          return json({ success: false, error: 'Token não pertence a este aplicativo' }, 401);
        }
        ...
        if (!ud.email) {
          return json({ success: false, error: 'Email não encontrado no token Google' }, 400);
        }
        googleUser = { email: ud.email, name: ud.name || 'Google User', picture: ud.picture };
      }
    } catch (err) {
      logger.error('Google validation error:', err instanceof Error ? err.message : err);
    ...
    if (!dbUser) {
      const r = await env.DB.prepare(
        ...
        .bind(googleUser.email.toLowerCase(), googleUser.name, googleUser.picture || null)
        .run();
      dbUser = {
  ...
  try {
    ...
    try {
      if (env.FACEBOOK_APP_ID && env.FACEBOOK_APP_SECRET) {
        ...
        const debug = (await debugResp.json()) as {
          data?: { is_valid?: boolean; app_id?: string };
        };
        if (!debug.data?.is_valid || debug.data?.app_id !== env.FACEBOOK_APP_ID) {
          return json(
            { success: false, error: 'Token Facebook inválido ou não pertence a este aplicativo' },
            401,
          );
        }
      }
      ...
      const picData = (fbData.picture as Record<string, unknown>)?.data as
        | Record<string, unknown>
        | undefined;
      fbUser = {
        email: String(fbData.email),
        name: String(fbData.name || 'Facebook User'),
        picture: picData?.url as string | undefined,
      };
    } catch (err) {
      logger.error('Facebook validation error:', err instanceof Error ? err.message : err);
    ...
    if (!dbUser) {
      const r = await env.DB.prepare(
        ...
        .bind(fbUser.email.toLowerCase(), fbUser.name, fbUser.picture || null)
        .run();
      dbUser = {
        id: r.meta.last_row_id,
