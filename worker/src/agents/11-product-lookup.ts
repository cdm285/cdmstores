...
const PRODUCTS: Array<{ id: number; name: string; price: number; stock: number; sku: string }> = [
  { id: 1, name: 'Fone Bluetooth', price: 89.9, stock: 50, sku: 'CDM-FONE-BT' },
  { id: 2, name: 'Carregador USB-C 65W', price: 49.9, stock: 100, sku: 'CDM-CHRG-65W' },
  { id: 3, name: 'Cabo Lightning 2m', price: 29.9, stock: 0, sku: 'CDM-CABLE-LT2' },
  { id: 4, name: 'Caixa de Som Portátil', price: 149.9, stock: 5, sku: 'CDM-SPK-PRT' },
];
...
function stockLabel(stock: number, lang: string): string {
  ...
  if (lang === 'es') {
    return stock > 0 ? `✅ En stock (${stock} unidades)` : '❌ Agotado';
  }
...
function singleProductResponse(p: (typeof PRODUCTS)[number], lang: string): string {
  const buy =
    { pt: 'Adicionar ao Carrinho', en: 'Add to Cart', es: 'Agregar al Carrito' }[lang] ??
    ...
    try {
      ...
      if (product_id) {
        ...
        if (!p) {
          ...
          const notFound = {
            pt: `❌ Produto #${product_id} não encontrado.`,
            en: `❌ Product #${product_id} not found.`,
            es: `❌ Producto #${product_id} no encontrado.`,
          };
          return {
            id: req.id,
            actionType: 'product_lookup',
            success: false,
            response: notFound[lang] ?? notFound.pt,
            error: 'not_found',
            latencyMs: Date.now() - start,
            ts: Date.now(),
          };
        ...
        const result: ActionResult = {
          ...
          response,
          data: p,
          action: p.stock > 0 ? 'add_to_cart' : 'notify_stock',
          actionPayload: { product_id: p.id, product_name: p.name, product_price: p.price },
          latencyMs: Date.now() - start,
          ts: Date.now(),
        };
        addTrace(ctx, {
        ...
        const matches = PRODUCTS.filter(
          p =>
            p.name
              ...
              .normalize('NFD')
              .replace(/[\u0300-\u036f]/g, '')
              .includes(q) || p.sku.toLowerCase().includes(q),
        );
