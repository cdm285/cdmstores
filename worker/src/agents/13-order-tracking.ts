    ...
    try {
      ...
      if (tracking_code) {
        order = await env.DB.prepare(
          'SELECT id, status, tracking_code, created_at, updated_at, customer_email, total FROM orders WHERE tracking_code = ? LIMIT 1',
        )
      ...
      if (!order && order_id) {
        order = await env.DB.prepare(
          'SELECT id, status, tracking_code, created_at, updated_at, customer_email, total FROM orders WHERE id = ? LIMIT 1',
        )
          .bind(order_id)
          .first<OrderRow>();
      }
      ...
      if (!order && email) {
        order = await env.DB.prepare(
          'SELECT id, status, tracking_code, created_at, updated_at, customer_email, total FROM orders WHERE customer_email = ? ORDER BY created_at DESC LIMIT 1',
        )
      ...
      const result: ActionResult = {
        ...
        actionPayload: {
          data: { id: order.id, status: order.status, tracking_code: order.tracking_code },
        },
        latencyMs: Date.now() - start,
        ts: Date.now(),
      };
