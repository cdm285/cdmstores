...
function buildUpdateSQL(table: string, data: Record<string, unknown>, where: Record<string, unknown>): { sql: string; values: unknown[] } {
...
export class Agent16DatabaseWrite {
  ...
  async execute(ctx: ExtendedAgentContext, req: ActionRequest): Promise<ActionResult> {
    ...
    if (!isAllowedTable(table)) {
      ...
      addTrace(ctx, { agentId: this.id, agentName: this.name, success: false, latencyMs: 0, error: msg });
    ...
    if (colError) {
      addTrace(ctx, { agentId: this.id, agentName: this.name, success: false, latencyMs: 0, error: colError });
    ...
    try {
      ...
      if (operation === 'insert') {
        ...
        result = await env.DB.prepare(sql).bind(...values).run();
      }
      ...
      const actionResult: ActionResult = {
        ...
        data         : { table, operation, rowsAffected: (result as D1Result & { rowsAffected?: number }).rowsAffected ?? 1 },
        action       : `db_${operation}`,
        latencyMs    : Date.now() - start,
        ts           : Date.now(),
      };
      addTrace(ctx, { agentId: this.id, agentName: this.name, success: true, latencyMs: actionResult.latencyMs });
      return actionResult;
