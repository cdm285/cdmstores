...
const TEMPLATES: Record<
  string,
  (vars: Record<string, string>, lang: string) => { subject: string; html: string }
> = {
  order_shipped: (vars, lang) => {
    ...
    if (lang === 'en') {
      return {
        subject: `Your order #${ord} has been shipped!`,
        html: `<p>Great news! Your order <b>#${ord}</b> is on its way.<br>Tracking code: <b>${trk}</b></p>`,
      };
    }
    if (lang === 'es') {
      return {
        subject: `¡Tu pedido #${ord} fue enviado!`,
        html: `<p>¡Buenas noticias! Tu pedido <b>#${ord}</b> está en camino.<br>Código de seguimiento: <b>${trk}</b></p>`,
      };
    }
    return {
      subject: `Seu pedido #${ord} foi enviado!`,
      html: `<p>Ótimas notícias! Seu pedido <b>#${ord}</b> está a caminho.<br>Código de rastreio: <b>${trk}</b></p>`,
    };
  },
  coupon_applied: (vars, lang) => {
    ...
    if (lang === 'en') {
      return {
        ...
        html: `<p>Your coupon <b>${code}</b> was applied. You saved <b>R$ ${disc}</b> 🎉</p>`,
      };
    }
    if (lang === 'es') {
      return {
        subject: `¡Cupón ${code} aplicado!`,
        html: `<p>Tu cupón <b>${code}</b> fue aplicado. ¡Ahorraste <b>R$ ${disc}</b> 🎉</p>`,
      };
    }
    return {
      subject: `Cupom ${code} aplicado!`,
      html: `<p>Seu cupom <b>${code}</b> foi aplicado. Você economizou <b>R$ ${disc}</b> 🎉</p>`,
    };
  },
  support_reply: (vars, lang) => {
    ...
    if (lang === 'en') {
      return {
        subject: 'Our support team has replied',
        html: `<p>Our team responded to your request:<br><br><em>${msg}</em></p>`,
      };
    }
    if (lang === 'es') {
      return {
        subject: 'Nuestro equipo de soporte respondió',
        html: `<p>Nuestro equipo respondió a tu solicitud:<br><br><em>${msg}</em></p>`,
      };
    }
    return {
      subject: 'Nossa equipe de suporte respondeu',
      html: `<p>Nossa equipe respondeu à sua solicitação:<br><br><em>${msg}</em></p>`,
    };
  },
  generic: (vars, lang) => {
    const msg =
    ...
    if (lang === 'en') {
      return { subject: 'CDM STORES Notification', html: `<p>${msg}</p>` };
    }
    if (lang === 'es') {
      return { subject: 'Notificación CDM STORES', html: `<p>${msg}</p>` };
    }
    ...
    try {
      ...
      if (channel === 'email') {
        if (!EMAIL_SAFE_RE.test(recipient)) {
          return failedResult(
            req,
            `Invalid email address: ${recipient.slice(0, 50)}`,
            Date.now() - start,
          );
        }
        ...
        if (!env.RESEND_API_KEY) {
          ...
          const result: ActionResult = {
            ...
            success: true,
            response: dryMsg[lang] ?? dryMsg.pt,
            data: { channel, recipient, template, dryRun: true },
        ...
        const result: ActionResult = {
          ...
          actionType: 'notification_send',
          success: true,
          response: okMsg[lang] ?? okMsg.pt,
          data: { channel, recipient, emailId: send.id },
          action: 'notification_sent',
          actionPayload: { channel, recipient },
          latencyMs: Date.now() - start,
          ts: Date.now(),
        };
        addTrace(ctx, {
      ...
      if (channel === 'whatsapp') {
        const text =
          safeVars.message ??
          (lang === 'en'
            ? 'You have a new notification from CDM STORES.'
            : lang === 'es'
              ? 'Tienes una nueva notificación de CDM STORES.'
              : 'Você tem uma nova notificação da CDM STORES.');
        ...
        const result: ActionResult = {
          ...
          actionType: 'notification_send',
          success: true,
          response: msg[lang] ?? msg.pt,
          data: { channel, link },
          action: 'notification_sent',
          actionPayload: { channel, link },
          latencyMs: Date.now() - start,
          ts: Date.now(),
        };
        addTrace(ctx, {
      ...
      const result: ActionResult = {
        ...
        actionType: 'notification_send',
        success: true,
        response: pushMsg[lang] ?? pushMsg.pt,
        data: { channel, recipient, template, stub: true },
        action: 'notification_sent',
        latencyMs: Date.now() - start,
        ts: Date.now(),
      };
