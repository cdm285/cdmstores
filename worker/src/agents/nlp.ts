...
import type { AgentContext, AgentResult, Intent, IntentCategory } from '../core/types.js';
...
export class IntentAgent extends BaseAgent {
  ...
  private static readonly RULES: Array<{
    ...
    },
    {
      category: 'tracking',
      patterns: [
        /\b(rastr\w*|track\w*|onde está|onde ta|cadê|status do pedido|entrega|delivered)\b/i,
      ],
      route: 'fast',
      confidence: 88,
    },
    ...
    },
    {
      ...
      patterns: [
        /\b(produtos?|products?|quanto custa|preço|price|precio|fone|carregador|cabo|caixa)\b/i,
      ],
      route: 'fast',
      confidence: 85,
    },
    ...
    },
    {
      category: 'support',
      patterns: [
        /\b(problema|erro|bug|ajuda|help|suporte|support|não funciona|quebrado|defeito)\b/i,
      ],
      route: 'full',
      confidence: 82,
    },
...
export class LanguageAgent extends BaseAgent {
  ...
  private static readonly PT =
    /\b(eu|você|ele|ela|nós|eles|que|não|sim|mas|com|para|por|uma|um|isso|este|aqui)\b/i;
  private static readonly EN =
    /\b(I|you|he|she|we|they|the|is|are|was|were|have|has|do|does|not|yes|but|with|for|this|that|here)\b/i;
  private static readonly ES =
    /\b(yo|tú|él|ella|nosotros|ellos|que|no|sí|pero|con|para|por|una|un|esto|ese|aquí)\b/i;
