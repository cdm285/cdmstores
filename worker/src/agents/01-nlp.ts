...
const PRODUCT_MAP: Array<{
...
}> = [
  {
    id: 1,
    name: 'Fone Bluetooth',
    triggers: ['fone', 'headphone', 'auricular', 'bluetooth headphone'],
  },
  {
    id: 2,
    name: 'Carregador USB-C 65W',
    triggers: ['carregador', 'charger', 'cargador', 'usb', 'usbc'],
  },
  { id: 3, name: 'Cabo Lightning 2m', triggers: ['cabo', 'cable', 'lightning', 'cabo lightning'] },
  {
    id: 4,
    name: 'Caixa de Som Portátil',
    triggers: ['caixa', 'speaker', 'parlante', 'caixa de som', 'som portátil'],
  },
];
...
export class Agent01NLP {
  ...
  async execute(ctx: ExtendedAgentContext, message: string): Promise<void> {
    ...
    if (emailM) {
      entities.email = emailM[1].toLowerCase();
    }

    // Tracking code
    const trackM = normalized.match(PATTERNS.tracking);
    if (trackM) {
      entities.tracking_code = trackM[1].toUpperCase();
    }
    ...
    for (const product of PRODUCT_MAP) {
      for (const trigger of product.triggers) {
        if (lower.includes(trigger) && trigger.length > bestTrigLen) {
          bestProductId = product.id;
          bestProductName = product.name;
          bestTrigLen = trigger.length;
        }
      }
    }
    if (bestProductId !== null) {
      entities.product_id = bestProductId;
      entities.product_name = bestProductName ?? '';
    }

    // Quantity hint ("2 fones", "três cabos")
    const qtyM = lower.match(/\b([2-9]|[1-9]\d)\s+(?:fone|carregador|cabo|caixa)/);
    if (qtyM) {
      entities.quantity = Number(qtyM[1]);
    }
