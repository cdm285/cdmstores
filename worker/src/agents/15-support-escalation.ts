...
function whatsappMsg(lang: string): string {
  if (lang === 'en') {
    return 'Hello! I need help with my CDM STORES order.';
  }
  if (lang === 'es') {
    return '¡Hola! Necesito ayuda con mi pedido de CDM STORES.';
  }
...
function buildEscalationResponse(reason: string, lang: string): string {
  ...
  if (reason === 'negative_sentiment') {
    if (lang === 'en') {
      return `😔 I can see you're having trouble. Let me connect you to our support team.\n\n💬 [Chat on WhatsApp](${link})\n📧 ${SUPPORT_EMAIL}\n\nWe respond within 2 business hours.`;
    }
    if (lang === 'es') {
      return `😔 Veo que estás teniendo dificultades. Te conecto con nuestro equipo de soporte.\n\n💬 [Chat en WhatsApp](${link})\n📧 ${SUPPORT_EMAIL}\n\nRespondemos en 2 horas hábiles.`;
    }
