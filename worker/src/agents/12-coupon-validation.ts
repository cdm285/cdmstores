...
const COUPONS: Record<string, { discount: number; minOrder?: number; maxUses?: number }> = {
  NEWYEAR    : { discount: 10.00 },
  PROMO      : { discount:  5.00 },
  DESCONTO10 : { discount: 10.00, minOrder: 50 },
  SAVE20     : { discount: 20.00, minOrder: 100 },
  CDM10      : { discount: 10.00 },
};
...
export class Agent12CouponValidation {
  ...
  async execute(ctx: ExtendedAgentContext, req: ActionRequest): Promise<ActionResult> {
    ...
    try {
      ...
      if (staticEntry) {
        // Check minimum order
        if (staticEntry.minOrder !== undefined && cart_total !== undefined && cart_total < staticEntry.minOrder) {
          ...
          const result: ActionResult = {
            id: req.id, actionType: 'coupon_validate', success: false,
            response: minMsg[lang] ?? minMsg.pt,
            action: 'coupon_applied',
            actionPayload: { coupon_valid: false, discount: 0, code: upper, reason: 'min_order' },
            latencyMs: Date.now() - start, ts: Date.now(),
          };
          addTrace(ctx, { agentId: this.id, agentName: this.name, success: false, latencyMs: result.latencyMs, error: 'min_order_not_met' });
          return result;
        }
        ...
        const result: ActionResult = {
          id: req.id, actionType: 'coupon_validate', success: true,
      ...
      if (env.DB) {
        try {
          const row = await env.DB
            .prepare('SELECT code, discount_amount, min_order, active FROM coupons WHERE code = ? AND active = 1 LIMIT 1')
            .bind(upper)
            .first<{ code: string; discount_amount: number; min_order: number | null; active: number }>();
          ...
          if (row) {
            if (row.min_order !== null && cart_total !== undefined && cart_total < row.min_order) {
              ...
              const result: ActionResult = { id: req.id, actionType: 'coupon_validate', success: false, response: minMsg[lang] ?? minMsg.pt, action: 'coupon_applied', actionPayload: { coupon_valid: false, discount: 0, code: upper, reason: 'min_order' }, latencyMs: Date.now() - start, ts: Date.now() };
              addTrace(ctx, { agentId: this.id, agentName: this.name, success: false, latencyMs: result.latencyMs, error: 'min_order_not_met' });
              return result;
            }
            ...
            const result: ActionResult = { id: req.id, actionType: 'coupon_validate', success: true, response: validMsg[lang] ?? validMsg.pt, data: { code: upper, discount: row.discount_amount }, action: 'coupon_applied', actionPayload: { coupon_valid: true, discount: row.discount_amount, code: upper }, latencyMs: Date.now() - start, ts: Date.now() };
            addTrace(ctx, { agentId: this.id, agentName: this.name, success: true, latencyMs: result.latencyMs });
            return result;
          }
      ...
      addTrace(ctx, { agentId: this.id, agentName: this.name, success: false, latencyMs: result.latencyMs, error: 'invalid_code' });
      return result;
