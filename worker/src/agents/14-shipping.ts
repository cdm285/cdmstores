...
const TIERS: ShippingTier[] = [
  { min: 1, max: 9, cost: 10.0, daysMin: 2, daysMax: 3, region: 'São Paulo Capital' },
  { min: 10, max: 19, cost: 12.0, daysMin: 3, daysMax: 5, region: 'São Paulo Interior' },
  { min: 20, max: 28, cost: 14.0, daysMin: 3, daysMax: 5, region: 'Rio de Janeiro' },
  { min: 29, max: 39, cost: 16.0, daysMin: 4, daysMax: 6, region: 'ES / MG' },
  { min: 40, max: 48, cost: 18.0, daysMin: 5, daysMax: 7, region: 'Bahia' },
];
const DEFAULT_TIER: Omit<ShippingTier, 'min' | 'max' | 'region'> = {
  cost: 20.0,
  daysMin: 5,
  daysMax: 8,
};
...
function lookupTier(cep: string): {
  cost: number;
  daysMin: number;
  daysMax: number;
  region: string;
  ...
  return tier
    ...
    : {
        ...
        daysMin: DEFAULT_TIER.daysMin,
        daysMax: DEFAULT_TIER.daysMax,
        region: 'Brasil',
      };
}
...
function buildResponse(
  cep: string,
  tier: ReturnType<typeof lookupTier>,
  isFree: boolean,
  lang: string,
): string {
    ...
    try {
      ...
      const result: ActionResult = {
        ...
        actionPayload: {
          postal_code: cep,
          shipping_cost: cost,
          estimated_days_min: tier.daysMin,
          estimated_days_max: tier.daysMax,
        },
