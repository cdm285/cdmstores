...
async function dbRead(env: AgentEnv, sessionId: string): Promise<Record<string, unknown> | null> {
  try {
    ...
    if (row.expires_at < nowS) {
      ...
      env.DB.prepare('DELETE FROM session_cache WHERE session_id = ?')
        .bind(sessionId)
        .run()
        .catch(() => void 0);
      return null;
...
async function dbWrite(
  ...
  await env.DB.prepare(
    ...
    .bind(sessionId, JSON.stringify(data), expiresAt)
    .run();
}
        ...
        case 'read': {
          ...
          if (cached) {
            ...
            for (const [k, v] of Object.entries(rest)) {
              if (k !== 'updatedAt') {
                (ctx.meta as Record<string, unknown>)[k] = v;
              }
            }
